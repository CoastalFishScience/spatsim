...
#df <- mixTable("MixingModels/mm_results/snook_ss11232022.txt",'year',ind = F,nest = F)
#df <- df [-c(8:12)]
...
df <- read.csv("df_with_year_ss.csv") # year should be the type
...
#write_csv(df_hvAll, "/Users/mack/Desktop/RESEARCH/Manuscripts_R Scripts/movement/spatial similarity manuscript/snook_similarity_iterations_1122022.csv")
# FORMAT AND BROUGHT BACK IN FOR GGPLOT FIGURE #2

long_df <- read.csv("snook_similarity_long_11222022.csv")
...
tiff("~hvfigureone_11222022.tiff", width = 9, height = 9, units = 'in', 
...
tiff("~snook_sim_11232022.tiff", width = 9, height = 9, units = 'in', 
     res = 600, compression = 'lzw')

ggplot(long_df, aes(x=wYear, y=vol, group=year)) +
...
tiff("~wYearVolByYear.tiff", width = 9, height = 9, units = 'in', 
     res = 600, compression = 'lzw')
