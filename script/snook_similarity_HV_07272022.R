setwd("~/Desktop/SPATIAL SIMILARITY PAPER")
...
#df <- mixTable("MixingModels/mm_results/snook_ss07192022.txt",'year',ind = F,nest = F)
#df <- df [-c(8:12)]
#write_csv(df, "/Users/mack/Desktop/SPATIAL SIMILARITY PAPER/df_with_year_ss.csv")
df <- read.csv("df_with_year_ss.csv")
...
df_hvAll = tibble(rep = rep(seq(1,n,1)),
                  vol_2012 = NA, vol_2013 = NA, vol_2014 = NA, vol_2015 = NA,
                  vol_2017 = NA, vol_2018 = NA, vol_2019 = NA, vol_2020 = NA, vol_2021 = NA)
...
for (i in 1:n){
      ...
      hv_2012 = hypervolume_gaussian(df1, name = '2012',
      ...
      # generate hypervolume
      hv_2013 = hypervolume_gaussian(df2, name = '2013',
                                     #samples.per.point = ceiling((10^(3 + sqrt(ncol(wet_df))))/nrow(wet_df)),
                                     samples.per.point = 5000,
      ...
      # generate hypervolume
      hv_2014 = hypervolume_gaussian(df3, name = '2014',
                                     #samples.per.point = ceiling((10^(3 + sqrt(ncol(wet_df))))/nrow(wet_df)),
                                     samples.per.point = 5000,
      ...
      # generate hypervolume
      hv_2015 = hypervolume_gaussian(df4, name = '2015',
                                     #samples.per.point = ceiling((10^(3 + sqrt(ncol(wet_df))))/nrow(wet_df)),
                                     samples.per.point = 5000,
      ...
      # get volume
      df_hvAll$vol_2015[i] = get_volume(hv_2015)
      ...
      # generate hypervolume
      hv_2017 = hypervolume_gaussian(df5, name = '2017',
                                     #samples.per.point = ceiling((10^(3 + sqrt(ncol(wet_df))))/nrow(wet_df)),
                                     samples.per.point = 5000,
      ...
      # generate hypervolume
      hv_2018 = hypervolume_gaussian(df6, name = '2018',
                                     #samples.per.point = ceiling((10^(3 + sqrt(ncol(wet_df))))/nrow(wet_df)),
                                     samples.per.point = 5000,
      ...
      # generate hypervolume
      hv_2019 = hypervolume_gaussian(df7, name = '2019',
                                     #samples.per.point = ceiling((10^(3 + sqrt(ncol(wet_df))))/nrow(wet_df)),
                                     samples.per.point = 5000,
      ...
      # generate hypervolume
      hv_2020 = hypervolume_gaussian(df8, name = '2020',
                                     #samples.per.point = ceiling((10^(3 + sqrt(ncol(wet_df))))/nrow(wet_df)),
                                     samples.per.point = 5000,
      ...
      # generate hypervolume
      hv_2021 = hypervolume_gaussian(df9, name = '2021',
                                     #samples.per.point = ceiling((10^(3 + sqrt(ncol(wet_df))))/nrow(wet_df)),
                                     samples.per.point = 5000,
...
#write_csv(df_hvAll, "/Users/mack/Desktop/SPATIAL SIMILARITY PAPER/snook_similarity_iterations_07262022.csv")
# FORMAT AND BROUGHT BACK IN FOR GGPLOT FIGURE #2

setwd("~/Desktop/RESEARCH/movement/spatial similarity manuscript")

long_df <- read.csv("snook_similiarity_long_07262022.csv")

long_df$wYear<-factor(long_df$wYear, levels = c("2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020"))

#### PRODUCT NUMBER ONE = BOOTSTRAPPED HYPERVOLUME FIGURE
hvBS= hypervolume_join(hv_2012, hv_2013, hv_2014, hv_2015, hv_2017,
                       hv_2018, hv_2019, hv_2020, hv_2021)

tiff("/Users/mack/Desktop/SPATIAL SIMILARITY PAPER/hvfigureone_07262022.tiff", width = 9, height = 9, units = 'in', 
     res = 600, compression = 'lzw')
plot(hvBS, pairplot = T,
     names= c(expression(italic("Bay"),italic("Ecotone"), italic("Freshwater"))),
...
tiff("/Users/mack/Desktop/SPATIAL SIMILARITY PAPER/snook_sim_07262022.tiff", width = 9, height = 9, units = 'in', 
     res = 600, compression = 'lzw')

ggplot(long_df, aes(x=year, y=vol, group=year)) +
      geom_violin(trim = FALSE) +
      geom_boxplot(width =0.2) +
      labs(title = "Annual Dry Season Common Snook Niche Volume", x = "Year", 
           y = "Niche Volume") +
...
tiff("/Users/mack/Desktop/RESEARCH/movement/spatial similarity manuscript/wYearVolByYear.tiff", width = 9, height = 9, units = 'in', 
     res = 600, compression = 'lzw')

ggplot(long_df, aes(x=wYear, y=vol, group=wYear)) +
